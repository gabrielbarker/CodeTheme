...
exports.setSettingsFileToRandomTheme = function() {
    ...
    x.getRandomColourPalette()
        .then((colourStrings) => {
            ...
            settings["editor.tokenColorCustomizations"]["[Visual Studio Dark]"] = jsonEditorTheme;
            settings["workbench.colorCustomizations"]["[Visual Studio Dark]"] = jsonWorkbenchTheme;
...
exports.saveCurrentTheme = function(name) {
    ...
    if(uniqueName(name, savedThemesJSON)) {
        ...
        namedThemeJSON["settings"]["editor.tokenColorCustomizations"] = settingsJSON["editor.tokenColorCustomizations"];
        namedThemeJSON["settings"]["workbench.colorCustomizations"] = settingsJSON["workbench.colorCustomizations"];
...
exports.setThemeToSavedTheme = function(name) {
    ...
    savedThemesJSON.themes.forEach(t => 
        {
            if(t.name === name) {
                settings["editor.tokenColorCustomizations"] = t.settings["editor.tokenColorCustomizations"];
                settings["workbench.colorCustomizations"] = t.settings["workbench.colorCustomizations"];
                writeSettingsFile(settings);
                themeExists = true;
            }
        });
...
exports.createSlackTheme = function() {
    let colours = [];
    let slackString = "";
    x.getRandomColourPalette().then((colourStrings) => {
        colourStrings.forEach(c => {
            colours.push(Colour(c));
        });
        colours = orderColoursByLightness(colours);
        slackString = `${colours[0].lighten(0.2).hex()},${colours[3].hex()},${colours[1].hex()},` 
                    + `${colours[3].hex()},${colours[0].hex()},${colours[1].lighten(0.5).saturate(0.3).hex()},` 
                    + `${colours[4].hex()},${colours[3].hex()}`;
        console.log();
