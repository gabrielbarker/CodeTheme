...
const savedThemesPath = '/Users/gbarker/SideProjects/codeTheme/lib/savedThemes.json';
const fs = require('fs');
...
function createJSONWorkbenchTheme(colours) {
    return jsonString = {
        ...
        "foreground": `${colours[0].hex()}`,
        "widget.shadow": `${colours[0].darken(0.6).hex()}`,
        "selection.background": `${colours[0].darken(0.7).hex()}`,
        "descriptionForeground": `${colours[0].darken(0.6).hex()}`,
        "errorForeground": `${colours[0].darken(0.6).hex()}`,
        ...
        "menu.foreground": `${colours[0].darken(0.7).hex()}`,
...
function readDefaultSettings() {
    const raw = fs.readFileSync('/Users/gbarker/SideProjects/codeTheme/lib/default.json');
    return JSON.parse(raw);
}
