const settingsPath =
  "/Users/gbarker/Library/Application Support/Code/User/settings.json";
const savedThemesPath = __dirname + "/savedThemes.json";
const fs = require("fs");
...
function setEditorAndWorkbench(settings, newSettings) {
  settings["editor.tokenColorCustomizations"] =
    newSettings["editor.tokenColorCustomizations"];
  settings["workbench.colorCustomizations"] =
...
exports.createSlackTheme = function() {
  ...
  x.getRandomColourPalette().then(colourStrings => {
    ...
    slackString =
      `${colours[0]
        .lighten(0.2)
        .hex()},${colours[3].hex()},${colours[1].hex()},` +
      `${colours[3].hex()},${colours[0].hex()},${colours[1]
        .lighten(0.5)
        .saturate(0.3)
        .hex()},` +
